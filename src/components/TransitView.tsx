...
import { Compass, AlertCircle, Award, Sparkles, Layers } from 'lucide-react';
import { GocharaRahuView } from './GocharaRahuView';
...
export const TransitView: React.FC<TransitViewProps> = ({ transit, language }) => {
  const [activeTransitMode, setActiveTransitMode] = useState<'rahu_nadi' | 'all_transits'>('rahu_nadi');

  return (
    <div className="space-y-6">
      {/* Top Transit Selector Switch */}
      <div className="flex items-center justify-between flex-wrap gap-3 bg-slate-900/90 p-3 rounded-2xl border border-slate-800 shadow-xl">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-bold text-amber-200 font-serif">
              {language === 'ta' ? 'கோசார ஆய்வு மையம் (Gochara Planetary Transit)' : 'Gochara Planetary Transit Center'}
            </h2>
            <p className="text-[11px] text-slate-400">
              {language === 'ta' ? 'நாடி ராகு கோசாரம் மற்றும் 9 கிரகங்களின் தற்போதைய பலன்கள்' : 'Nadi Rahu transit treatise & real-time planetary transits'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-slate-950 p-1.5 rounded-xl border border-slate-800">
          <button
            onClick={() => setActiveTransitMode('rahu_nadi')}
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all ${
              activeTransitMode === 'rahu_nadi'
                ? 'bg-amber-500 text-slate-950 shadow-md font-semibold'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Sparkles className="w-3.5 h-3.5 text-amber-950" />
            <span>{language === 'ta' ? 'கோள்சார ராகு நாடிப் பலன்கள்' : 'Gochara Rahu (Nadi Treatise)'}</span>
          </button>

          <button
            onClick={() => setActiveTransitMode('all_transits')}
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all ${
              activeTransitMode === 'all_transits'
                ? 'bg-amber-500 text-slate-950 shadow-md font-semibold'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Layers className="w-3.5 h-3.5" />
            <span>{language === 'ta' ? 'அனைத்து கிரகக் கோசாரம்' : 'All Planet Transits'}</span>
          </button>
        </div>
      </div>

      {activeTransitMode === 'rahu_nadi' && (
        <GocharaRahuView rahuAnalysis={transit.rahuNadiAnalysis} language={language} />
      )}
